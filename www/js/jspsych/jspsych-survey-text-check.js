...
jsPsych.plugins['survey-text-check'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-text-check',
    description: '',
    parameters: {
      questions: {
        ...
        nested: {
          ...
          input_type: {
            ...
            default: "text",
            description: 'The type for the response text box.'
          },
          rows: {
          ...
          columns: {
            ...
            pretty_name: 'Step',
            default: undefined,
            description: 'Step size for numeric fields.'
          }, min: {
            type:  jsPsych.plugins.parameterType.FLOAT,
            pretty_name: "Min",
            default: undefined,
            description: "Minimum value for numeric input."
          }, max: {
            type: jsPsych.plugins.parameterType.FLOAT,
            pretty_name: "Max",
            default: undefined,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].type == "number") {
        html += '<input type="' + trial.questions[i].type + '" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'" step="'+trial.questions[i].step+'" min="'+trial.questions[i].min+'" max="'+trial.questions[i].max+'"></input>'
      } else {
        if(trial.questions[i].rows == 1){
          html += '<input type="' + trial.questions[i].type + '" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'"></input>';
        } else {
